...
let HttpErrorFilter = class HttpErrorFilter {
        ...
        const errorResponce = {
            ...
            message: (status !== common_1.HttpStatus.INTERNAL_SERVER_ERROR) ? exception.message.error || exception.message || null : 'Internal server error',
        };
        if (status === common_1.HttpStatus.INTERNAL_SERVER_ERROR) {
            console.error(exception);
        }
        common_1.Logger.error(`${request.method} ${request.url}`, JSON.stringify(errorResponce), 'ExceptionFilter');
